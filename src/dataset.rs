...
fn parse_images(path: &Path) -> Result<(Tensor, usize)> {
    ...
    let count = usize::try_from(read_u32(&mut file)?).expect("image count overflows usize");
...
fn parse_labels(path: &Path) -> Result<Vec<u8>> {
    ...
    let count = usize::try_from(read_u32(&mut file)?).expect("label count overflows usize");
